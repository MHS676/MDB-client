...
import AttendanceDetailsPage from '../dashboard/AttendanceDetailsPage'; // Import new Attendance page
...
const FalconDashboard = ({ user, onSignOut }) => {
  ...
  const navigationStructure = [
    {
      ...
      items: [
        ...
        { id: 'Guard Attendance Details', label: 'Guard Attendance Details', icon: '📋' }, // Added item
      ]
    },
    {
  ...
  const renderActivePage = () => {
    switch (currentPage) {
      ...
      case 'Guard Attendance Details':
        return <AttendanceDetailsPage />; // Active view rendering
        ...
        <header className="h-20 bg-[#0f172a] border-b border-slate-800 px-8 flex items-center justify-between shrink-0 select-none shadow-lg m-4 rounded-xl">
          ...
          <div className="flex items-center gap-8">
            ...
            <div className="flex items-center gap-3 border-l border-slate-700 pl-6">
              <button
                onClick={onSignOut}
                className="px-2.5 py-1 text-[10px] font-bold text-rose-400/80 hover:text-rose-400 hover:bg-rose-500/5 border border-rose-500/20 hover:border-rose-500/30 rounded-lg transition-all flex items-center gap-1.5"
                title="Logout"
              >
