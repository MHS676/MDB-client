  ...
  useEffect(() => {
    const fetchPeriodData = async () => {
      try {
        ...
        if (data) {
          setFormData({
            onIncome: data.tdsIncomeCash || '',
            onExpenditure: data.tdsExpenditureCash || '',
            paid: data.tdsIncomeBank || '', // map paid to tdsIncomeBank as per schema payload
          });
        } else {
          setFormData({ onIncome: '', onExpenditure: '', paid: '' });
        }
      } catch (err) {
  ...
  const handleSave = async () => {
    try {
      ...
      const payload = {
        ...
        tdsIncomeCash: parseFloat(formData.onIncome) || 0,
        tdsExpenditureCash: parseFloat(formData.onExpenditure) || 0,
        tdsIncomeBank: parseFloat(formData.paid) || 0,
      };

      await financialRecordsAPI.save(payload);
      alert('✓ TDS records saved successfully!');
      handleReset();
    } catch (err) {
      console.error('Error saving TDS data:', err);
      setError(err.message || 'Failed to save TDS data');
  ...
  return (
    <div className="h-[calc(100vh-140px)] flex flex-col justify-between max-w-4xl mx-auto space-y-4 overflow-hidden select-none">
      ...
      <div className="flex items-center justify-between shrink-0">
        <div>
          <h2 className="text-xl font-bold text-slate-800 tracking-tight">TDS Compliance</h2>
          <p className="text-xs text-slate-400 mt-0.5">Manage and monitor value-added tax profiles for corporate operations.</p>
        </div>
      ...
      <div className="flex-1 min-h-0 bg-white rounded-2xl border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.01)] p-8 flex flex-col justify-center items-center">
        ...
        <div className="w-full max-w-lg space-y-6">
          ...
          <div className="flex items-center justify-between gap-6">
            <label className="text-sm font-bold text-slate-600 uppercase tracking-wider w-36">
              on Income
            </label>
            <div className="relative flex-1">
              ...
              <input
                type="number"
                value={formData.onIncome}
                onChange={(e) => handleInputChange('onIncome', e.target.value)}
          ...
          <div className="flex items-center justify-between gap-6">
            ...
            <div className="relative flex-1">
              ...
              <input
                type="number"
                value={formData.onExpenditure}
                onChange={(e) => handleInputChange('onExpenditure', e.target.value)}
                className="w-full bg-slate-50/50 border border-slate-200 rounded-xl pl-9 pr-4 py-2.5 text-sm font-bold focus:bg-white focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all outline-none text-slate-800 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                placeholder="0.00"
              />
            </div>
