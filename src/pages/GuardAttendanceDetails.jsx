...
export default function GuardAttendanceDetails() {
  const attendanceBase = getConfig('EXTERNAL.ATTENDANCE') || 'http://localhost:5000/attendance';
  const postsBase = getConfig('EXTERNAL.POSTS') || 'http://localhost:5000/posts';
  ...
  // Fetch Attendance Summary on post or month change
  useEffect(() => {
    if (!selectedPost) {
      setSummary(null);
      return;
